...
def scrape_contacts(base_url):
    ...
    try:
        while queue and len(visited) < 5:
            ...
            if current == base_url:
                queue += find_links(base_url, html, ["contact", "about", "team", "support"])
    ...
    for _, v in results.items():
        final.append({
            ...
            "sources": ", ".join(v["urls"]),
            "email_count": len(v["emails"]),
            "phone_count": len(v["phones"])
        })
...
def extract_jobs(html):
    ...
    for tag in soup.find_all(['li', 'div', 'tr']):
        ...
        if any(word in text.lower() for word in ["apply", "engineer", "developer", "role", "position", "intern", "manager"]):
            jobs.append(text)
    structured = []
    for raw in set(jobs):
        structured.append({
            ...
            "technologies": extract_tech_stack(raw),
        })
    return pd.DataFrame(structured)

def extract_job_title(text):
    keywords = ["engineer", "developer", "manager", "intern", "analyst", "specialist"]
...
def scrape_jobs(base_url):
    ...
    try:
        ...
        if not res.ok:
            return pd.DataFrame()
        html = res.text
        links = find_links(base_url, html, ["careers", "jobs", "join-us", "work-with-us"])
        all_jobs = []
        for url in links:
            try:
                res2 = requests.get(url, headers=headers, timeout=10)
                if res2.ok:
                    job_df = extract_jobs(res2.text)
                    job_df["source"] = url
...
def full_scrape(url):
    ...
    if not jobs.empty:
        if not contacts.empty:
            ...
            jobs["phone"] = contacts["phones"].iloc[0] if not contacts["phones"].isnull().all() else ""
        else:
            jobs["email"] = ""
            jobs["phone"] = ""
